...
def initialize_browser():
    ...
    browser = webdriver.Chrome(
        r'D:\chromedriver_win32\chromedriver.exe', chrome_options=chrome_options)
...
def client_login(browser):
    try:
        browser.get('http://creis.fang.com')
        ...
        import pdb
        pdb.set_trace()
        handle_click(browser, '#btn_login > li:nth-child(1)')
        import pdb
        pdb.set_trace()
        handle_click(browser, '#cnproductselect > b.login_ico.ico_ch.fr')
        import pdb
        pdb.set_trace()
        handle_click(browser, '#cnproductlist > ul > li:nth-child(2) > a')
        import pdb
        pdb.set_trace()
        #定位到项目页
...
def prepare_statdata(browser, stattype, startdate, enddate, datetype, arrange=None):
    ...
    #清空输入框，初始化最新查询
    #handle_click(browser, '#ulLeftMenu > li:nth-child(2) > dl > dd:nth-child(1) > a')
    for stattype_key, stattype_value in stattype_locator_dict.items():
        if stattype_key != stattype:
            handle_click(browser, stattype_value)
            break
