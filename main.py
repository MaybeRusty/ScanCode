...
from spider import prepare_statdata, initialize_browser, close_browser, client_login, handle_click
...
if __name__ == "__main__":
    ...
    pdb.set_trace()
    #切换旧版
    handle_click(browser, '#app > div > ul > div > ul > li:nth-child(1) > a')
    ...
    enddate = datetime.datetime.strptime(
        '%s-%s-%s' % (2020, 5, 31), '%Y-%m-%d')
    startdate = datetime.datetime.strptime(
        '%s-%s-%s' % (2020, 5, 1), '%Y-%m-%d')
